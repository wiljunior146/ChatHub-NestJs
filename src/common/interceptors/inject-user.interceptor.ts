...
/**
 * Injecting user entity to a DTO with the specified request type.
 * 
...
export class InjectUserInterceptor implements NestInterceptor {
  /**
     ...
     * 
     * @note   This will be used for validation that requires other user entity
     *         properties like checking the current password in the request
     *         validation for updating the password.
  ...
  intercept(context: ExecutionContext, next: CallHandler): Observable<any> {
    ...
    if (this.to && request[this.to]) {
      request[this.to][REQUEST_CONTEXT] = request.user;
    }
